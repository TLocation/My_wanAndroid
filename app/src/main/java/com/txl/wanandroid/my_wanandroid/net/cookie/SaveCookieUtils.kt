...
import com.txl.wanandroid.my_wanandroid.App
...
class SaveCookieUtils {
    ...
    init {
        sharedPreferences = App.getInstance()!!.getSharedPreferences(COOKIE_SP_NAME, Context.MODE_PRIVATE)
        var all = sharedPreferences.all
        for ((key, value) in all) {
            var split = TextUtils.split(value as String, ",")
            for (item in split) {
                var encodeCookie = sharedPreferences.getString(item, null)
                if (encodeCookie != null) {
                    var decodeCookie = decodeCookie(encodeCookie)
